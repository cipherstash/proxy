    ...
    fn infer_exit(&mut self, this_expr: &'ast Expr) -> Result<(), TypeError> {
        match this_expr {
            ...
            Expr::IsDistinctFrom(a, b) | Expr::IsNotDistinctFrom(a, b) => {
                self.unify_node_with_type(this_expr, Type::native())?;
            ...
            Expr::BinaryOp { left, op, right } => {
                get_sql_binop_rule(op).apply_constraints(self, left, this_expr, right)?;
            }
            ...
            | Expr::Convert { expr, .. }
            | Expr::Cast { expr, .. } => {
                self.unify_nodes_with_type(this_expr, &**expr, Type::native())?;
            }
            ...
            Expr::Case {
                ...
                match operand {
                    ...
                    None => {
                        ...
                        self.unify_all_with_type(conditions, Type::native())?;
                    }
                }
