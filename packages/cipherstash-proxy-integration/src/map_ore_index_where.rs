...
mod tests {
    ...
    async fn map_ore_where_generic_int4() {
        map_ore_where_generic("encrypted_int4", 40i32, 42i32, 99i32).await;
    }

    #[tokio::test]
    async fn map_ore_where_generic_int8() {
        map_ore_where_generic("encrypted_int8", 40i64, 42i64, 99i64).await;
    }

    #[tokio::test]
    async fn map_ore_where_generic_float8() {
        map_ore_where_generic("encrypted_float8", 40.0f64, 42.0f64, 99.0f64).await;
    }

    #[tokio::test]
    async fn map_ore_where_generic_date() {
        let low = NaiveDate::parse_from_str("2024-01-01", "%Y-%m-%d").unwrap();
        let mid = NaiveDate::parse_from_str("2025-01-01", "%Y-%m-%d").unwrap();
        let high = NaiveDate::parse_from_str("2027-01-01", "%Y-%m-%d").unwrap();
        map_ore_where_generic("encrypted_date", low, mid, high).await;
    }
