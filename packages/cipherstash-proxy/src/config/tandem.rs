...
///
/// Expected string is in the form "missing field `{field}}` for key `{key}`"
///
...
mod tests {
    ...
    fn extract_field_and_key_name_from_config_error() {
        ...
        assert_eq!(key.unwrap(), "unknown".to_string());
    }
    ...
    fn missing_auth_config() {
        ...
        temp_env::with_vars(
            [
                ("CS_CLIENT_ID", Some("5912717c-2c3b-4fb6-a051-0a8e71cd9e37")),
                ("CS_CLIENT_KEY", Some("CS_CLIENT_KEY")),
                (
                    "CS_DEFAULT_KEYSET_ID",
                    Some("484cd205-99e8-41ca-acfe-55a7e25a8ec2"),
                ),
            ],
            || {
