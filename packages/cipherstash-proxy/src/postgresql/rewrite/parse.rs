...
use pg_proto::{FrontendMessage, Parse as PgParse};
...
mod tests {
    ...
    fn parse_fixture() -> PgParse {
        PgParse {
            statement: Bytes::new(),
            query: Bytes::from_static(
                b"INSERT INTO encrypted (id, encrypted_int2) VALUES ($1, $2)",
            ),
            parameter_types: vec![21, 21],
        }
    }
    ...
    pub fn test_parse() {
        ...
        let parse = Parse::from(expected.clone());
        let FrontendMessage::Parse(actual) = parse.into() else {
    ...
    pub fn test_parse_rewrite_param_types() {
        log::init(LogConfig::default());
        let mut parse = Parse::from(parse_fixture());
