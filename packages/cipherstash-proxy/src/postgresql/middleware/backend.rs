...
mod tests {
    ...
    async fn passthrough_reloads_changed_schema_before_ready_for_query() {
        ...
        let ready = BackendMessage::ReadyForQuery(pg_proto::codec::TransactionStatus::Idle);
        let mut disposition = BackendDisposition::Emit;
        let outbound = backend
            .intercept_backend(&mut disposition, ready.clone())
            .await
            .unwrap();
        reload_task.await.unwrap();
        assert_eq!(disposition, BackendDisposition::Emit);
        backend.write_with_flush(outbound).await.unwrap();
        assert_eq!(client_receiver.recv().await.unwrap(), ready);
