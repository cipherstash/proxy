...
set +e
OUTPUT="$(docker exec -i postgres${CONTAINER_SUFFIX} psql 'postgresql://cipherstash:p%40ssword@proxy:6432/cipherstash?sslmode=disable' --command 'SELECT id FROM encrypted WHERE encrypted_text IS NULL' 2>&1)"
retval=$?
...
set +e
OUTPUT="$(docker exec -i postgres${CONTAINER_SUFFIX} psql 'postgresql://cipherstash:p%40ssword@proxy:6432/cipherstash?sslmode=disable' --command "SELECT id FROM encrypted WHERE encrypted_text = 'Three'" 2>&1)"
retval=$?
