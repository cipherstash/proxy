...
docker exec -i postgres${CONTAINER_SUFFIX} psql postgresql://${CS_DATABASE__USERNAME}:p%40ssword@${CS_DATABASE__HOST}:${CS_DATABASE__PORT}/cipherstash <<-EOF
SELECT 1;
EOF

# Connect to the proxy
docker exec -i postgres${CONTAINER_SUFFIX} psql postgresql://cipherstash:p%40ssword@proxy:6432/cipherstash <<-EOF
SELECT 1;
EOF

# Connect to the proxy forcing TLS
docker exec -i postgres${CONTAINER_SUFFIX} psql 'postgresql://cipherstash:p%40ssword@proxy:6432/cipherstash?sslmode=require' <<-EOF
SELECT 1;
EOF

# Connect without TLS
set +e
OUTPUT="$(docker exec -i postgres${CONTAINER_SUFFIX} psql 'postgresql://cipherstash:p%40ssword@proxy:6432/cipherstash?sslmode=disable' --command 'SELECT 1' 2>&1)"
retval=$?
